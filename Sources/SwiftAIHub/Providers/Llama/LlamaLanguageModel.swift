...
import Foundation
  ...
  public final class LlamaLanguageModel: LanguageModel, @unchecked Sendable {
    ...
    public func streamResponse<Content>(
      ...
      // Only String streaming is supported. Return a throwing stream
      // instead of crashing the host when callers pass a non-String type.
      guard type == String.self else {
        return LanguageModelSession.ResponseStream(
          stream: AsyncThrowingStream { continuation in
            continuation.finish(
              throwing: LlamaLanguageModelError.structuredStreamingUnsupported
            )
          }
        )
      }
    ...
    private func resolvedStructuredOptions(from options: GenerationOptions)
    ...
    private func createContextParams(from options: ResolvedGenerationOptions)
    ...
    private func generateStructuredJSON(
      ...
      if options.repeatPenalty != 1.0 || options.frequencyPenalty != 0.0
      ...
      applySampling(
        sampler: samplerPointer, effectiveTemperature: options.temperature, options: options)
  ...
  public enum LlamaLanguageModelError: Error, LocalizedError {
    ...
    public var errorDescription: String? {
      ...
      case .structuredStreamingUnsupported:
        return "Structured streaming is not supported; only String content can be streamed"
      }
    }
  }
#endif  // Llama
