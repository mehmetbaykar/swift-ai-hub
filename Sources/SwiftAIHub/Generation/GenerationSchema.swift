    ...
    func encode(to encoder: Encoder) throws {
      ...
      case .object(let obj):
        ...
        for (name, node) in obj.properties {
          try propsContainer.encode(
            node, forKey: GenerationSchema.DynamicCodingKey(stringValue: name)!)
        }
    ...
    init(from decoder: Decoder) throws {
      ...
      case "object":
        ...
        for key in propsContainer.allKeys {
          properties[key.stringValue] = try propsContainer.decode(
            GenerationSchema.Node.self, forKey: key)
        }
        ...
        self = .object(
          GenerationSchema.ObjectNode(
            description: description, properties: properties, required: required)
        )
      ...
      case "string":
        ...
        self = .string(
          GenerationSchema.StringNode(
            description: description, pattern: pattern, enumChoices: enumChoices)
        )
    ...
    private static func buildNode<Value: Generable>(
      ...
      if type == Bool.self {
        ...
        return (
          .number(
            NumberNode(
              description: description, minimum: minimum, maximum: maximum, integerOnly: true)
          ), [:]
        )
        ...
        return (
          .number(
            NumberNode(
              description: description, minimum: minimum, maximum: maximum, integerOnly: false)
          ), [:]
        )
      } else {
