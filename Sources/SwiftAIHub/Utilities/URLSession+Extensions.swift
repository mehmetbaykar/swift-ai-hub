...
/// Redacts sensitive header values and bearer tokens from a raw HTTP error body
/// and caps its total size so it is safe to surface via logs / errors.
///
/// - Covers common auth headers (case-insensitive): Authorization, X-Api-Key,
///   api-key / apikey / Api-Key, Cookie, Set-Cookie, X-Auth-Token.
/// - Also scrubs any stray `Bearer <token>` substrings that may appear outside
///   of a header context (e.g. echoed inside a JSON error detail).
/// - Caps output at 4 KB (2 KB head + marker + 2 KB tail) so a verbose provider
///   cannot inflate memory footprints via leaked error bodies.
@Sendable
func redactSensitiveHeaders(_ raw: String) -> String {
  // 1) Redact sensitive header / JSON-field values.
  //    Matches both `Header: value` and `"api-key": "value"` style pairs.
  let headerPattern =
    #"(?i)(authorization|x-api-key|api[-_]?key|cookie|set-cookie|x-auth-token)(["']?\s*[:=]\s*["']?)[^\r\n"']+"#
  var sanitized = raw
  ...
  // 2) Scrub any stray `Bearer <token>` that slipped past header redaction.
  ...
  // 3) Cap size at 4 KB (head + tail).
  let maxBytes = 4096
  let utf8Count = sanitized.utf8.count
  if utf8Count > maxBytes {
    let halfBytes = maxBytes / 2
    let headEnd = sanitized.utf8.index(sanitized.utf8.startIndex, offsetBy: halfBytes)
    let tailStart = sanitized.utf8.index(sanitized.utf8.endIndex, offsetBy: -halfBytes)
    let head = String(sanitized.utf8[..<headEnd]) ?? ""
    let tail = String(sanitized.utf8[tailStart...]) ?? ""
    sanitized = head + "…[truncated]…" + tail
  }
